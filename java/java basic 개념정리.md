...
- gist로 제출할것
- <https://forms.gle/k92rHp6tVPAaDdu6A>
```

#### 1단계: 아는만큼 정리해서 적어보기..
```
1. 변수의 스코프
- 지역변수: 클래스 내부에서 일부 메서드(혹은 어떤 기능을 하는 로직) 내에서만 사용되는 변수. 메서드 사용이 끝나면 메모리 해제
- 전역변수: 클래스 변수로 클래스 안에 존재하는 전체 메서드에서 접근 및 변경 가능한 변수
- 클래스에서 선언된 int a와 클래스 내 메서드에서 선언된 int a는 다르다. (메모리 주소가 다르다.)
- 메서드 내에서 전역변수 접근 및 변경: this.전역변수

2. 접근 한정자
- public: 모든 클래스에서 접근 가능?
- private: 해당 클래스를 사용하는 객체에서만 접근 가능?
...
- List: 탐색속도에 강점을 갖고있는 순서가 있는 자료구조 ?
